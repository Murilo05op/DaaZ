class Map {
  ...
  public Map(int l, int a, String nomeArquivo, int tLar, int tAlt, PImage tImage, int nTiles){
    lar=l;
    alt=a;
    this.tAlt=tAlt;
    this.tLar=tLar;
  ...
  public void desenha(float offset){
    for(int i=0; i<alt; i++){
      for(int j=0;j<lar;j++){
        //desenhe o tile referenciado por layer[i][j] na tela
        if(layer[i][j]!=-1) image(tiles[layer[i][j]],j*tLar-cameraX/offset,i*tAlt-cameraY);
      }
    }
  }
}
